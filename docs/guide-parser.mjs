/**
 * Parses the framework reference guide (docs/src/guide.<locale>.html) into per-framework
 * blocks keyed by className.
 *
 * Two consumers share this: the app's Documentation tab (via
 * frontend/vite-plugin-framework-guide.ts) and the GitHub Pages generator (docs/build-site.mjs).
 ...
 * Both the outer .framework block and the inner .framework-body are found this way — using
...
/** Extracts the balanced <div class="framework">…</div> blocks. */
function extractBlocks(html) {
  const blocks = []
  const open = /<div class="framework">/g
  let m
...
export function parseGuide(html) {
  ...
  for (const block of extractBlocks(html)) {
    const className = pick(/class="framework-cls">([^<]*)</, block)
    if (!className) continue

    const tags = [...block.matchAll(/<span class="tag (tag-[a-z]+)">([^<]*)<\/span>/g)]
      .map(t => ({ kind: t[1], label: t[2].trim() }))

    // The body is everything inside .framework-body; inline styles there are print sizes in pt.
    const bodyStart = block.indexOf('<div class="framework-body">')
    ...
    out[className] = {
      number: pick(/class="framework-n">([^<]*)</, block),
...
/**
 * The `<div class="part">` headings that group the frameworks, in document order.
...
/** Frameworks in document order, each tagged with the part it belongs to. */
export function parseOutline(html) {
  ...
  while ((m = open.exec(html))) {
    const span = matchDiv(html, m.index)
    if (!span) break
    const className = pick(/class="framework-cls">([^<]*)</, html.slice(m.index, span.end))
    let part = null
